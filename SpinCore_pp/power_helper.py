...
def Ep_spacing_from_phalf(est_phalf = 0.2,sim_Emax = 30, max_power = 3.16,
           aspect_ratio = 1.618, p_steps = 20, min_dBm_step = 1.0, three_down = True, 
           fl = None):
    """
    Parameters
    ==========
...
Return
    ...
    dline = sp.simplify(sp.sqrt(1+(sp.diff(f/Emax,p)*(max_power/aspect_ratio))**2))
    if fl is not None:
    ...
    length_vs_p_fn = vectorize(
        ...
        excluded=[0])
    length_data = nddata(length_vs_p_fn(est_phalf,p_array),[-1],['p']).setaxis('p',p_array)
    length_data.invinterp('p', linspace(0,length_data['p',-1].item(),p_steps))
    if fl is not None:
        fl.plot(f_fn(sim_Emax,est_phalf,length_data.fromaxis('p')),'o',human_units = False)
        plt.text(0.5,0.5,'Just showing the E(p) to show JF the spacing is what we want - \nwe can delete this figure after we are good with everything else')
    W_settings = length_data.fromaxis('p').data
    rdB_settings = ones_like(W_settings)
    for x in range(len(W_settings)):
        if x == 0:
            rdB_settings[x] = 0.0
        else:    
            rdB_settings[x] = (
                round(10 * (log10(W_settings[x]) + 3.0) / min_dBm_step) * min_dBm_step
            )
    dB_settings = unique(rdB_settings)
    if three_down:
        append_dB = [
            rdB_settings[abs(10**(rdB_settings / 10.0 - 3) - max_power * frac).argmin()]
            for frac in [0.75, 0.5, 0.25]
        ]
        dB_settings = append(rdB_settings, append_dB)
    return dB_settings
#}}}
