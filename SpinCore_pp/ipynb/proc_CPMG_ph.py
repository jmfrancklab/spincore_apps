...
mpl.rcParams['figure.dpi']= 300 # good resolution figs
...
# Phasing, interleaving raw data ('pre'-processing)
...
# Copying phased, interleaved, and reordered data into separate nddata


# 


#d_T2 = interleaved['tE':(interleaved.getaxis('tE')[0],interleaved.getaxis('tE')[28])].C
d_T2 = interleaved.C
with figlist_var() as fl:
    for x in xrange(len(d_T2.getaxis('tE'))):
        fl.next('%s_%s \n all echoes'%(date,id_string))
...
# Create a plain numpy array from nddata
...
Nx = 100
Nx_ax = linspace(3e-20,0.3,Nx) # T2
tau1 = d_T2.getaxis('tE')
N1_2d = reshape(tau1,(shape(tau1)[0],1)) # T2 POINTS
Nx_2d = reshape(Nx_ax,(1,shape(Nx_ax)[0])) # T2 VALUES
k1 = exp(-(N1_2d)/Nx_2d)
...
for x in xrange(data.shape[1]):
    b[x,:] = data.real[:,x]
b_prime = zeros((b.shape[0],k1.shape[0]+k1.shape[1]))
...
# Perform loop over lambda values, looping along each frequency point and generate
# nnls solution at each point
...
figure();title('%s_%s \n Estimated F($T_{2}$,$\Omega$), $\lambda$ = %0.4f'%(date,id_string,heel_lambda))
...
figure(figsize=(14,10));suptitle('%s_%s'%(date,id_string))
subplot(221);subplot(221).set_title(r'abs data = $|d|$')
image(abs(d_T2))
subplot(222);subplot(222).set_title(r'abs fit = $|k_{1} x^{\intercal}|$')
image(abs(nd_fit))
subplots_adjust(hspace=.5)
subplot(223);subplot(223).set_title('residual = $d - k_{1} x^{\intercal}$')
image(nd_residual)
subplot(224);subplot(224).set_title('abs residual = $|d - k_{1} x^{\intercal}|$')
...
# Now repeat for 2nd experiment (duplicate of 1st...)
