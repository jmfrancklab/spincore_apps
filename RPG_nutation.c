...
int processArguments(int argc, char* argv[], SCANPARAMS* scanParams)
{
    ...
    scanParams->nPoints_Nutation = atoi (argv[3]);
    ...
    scanParams->nutation_step = atof (argv[14]);
...
int configureBoard(SCANPARAMS * scanParams)
{
    ...
    scanParams->nPoints_total = scanParams->nPoints_Nutation * scanParams->nPoints;
    ...
    ERROR_CATCH(spmri_set_num_segments(scanParams->nPoints_Nutation));
    return 0;
}
...
{
    char txt_fname[128];
    int* real = malloc(scanParams->nPoints * scanParams->nPoints_Nutation * sizeof(int));
    int* imag = malloc(scanParams->nPoints * scanParams->nPoints_Nutation * sizeof(int));
    int j;
    ERROR_CATCH(spmri_read_memory(real, imag, scanParams->nPoints * scanParams->nPoints_Nutation));
    // Text file output
    snprintf(txt_fname, 128, "%s.txt", scanParams->outputFilename);
    FILE* pFile = fopen( txt_fname, "w" );
    if ( pFile == NULL ) return -1;
    fprintf(pFile, "SPECTRAL WIDTH %f\n", scanParams->actualSW_Hz);
    for( j = 0 ; j < scanParams->nPoints * scanParams->nPoints_Nutation ; j++)
    {
        fprintf(pFile, "%d\t%d\n", real[j], imag[j]);
    }
    fclose(pFile);
