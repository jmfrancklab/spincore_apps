...
assert total_points < 2** 14, (
        "You are trying to acquire %d points (too many points) -- either change SW or acq time so nPoints x nPhaseSteps is less than 16384"%total_points)
#{{{ run ppg
with power_control() as p:
    ...
    FID = generic(
            ...
            indirect_len = len(dB_settings)+1, #make length long enough to include this no power
            indirect_idx = 0,
