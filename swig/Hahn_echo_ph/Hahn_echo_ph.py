...
if _swig_python_version_info >= (2, 7, 0):
    def swig_import_helper():
        ...
        mname = '.'.join((pkg, '_Hahn_echo_ph')).lstrip('.')
        try:
            return importlib.import_module(mname)
        except ImportError:
            return importlib.import_module('_Hahn_echo_ph')
    _Hahn_echo_ph = swig_import_helper()
    ...
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_Hahn_echo_ph', [dirname(__file__)])
        except ImportError:
            import _Hahn_echo_ph
            return _Hahn_echo_ph
        try:
            _mod = imp.load_module('_Hahn_echo_ph', fp, pathname, description)
        finally:
            if fp is not None:
                fp.close()
        return _mod
...
def get_time():
    return _Hahn_echo_ph.get_time()
get_time = _Hahn_echo_ph.get_time

def pause():
    return _Hahn_echo_ph.pause()
pause = _Hahn_echo_ph.pause

def configureTX(adcOffset, carrierFreq_MHz, tx_phases, amplitude, nPoints):
    return _Hahn_echo_ph.configureTX(adcOffset, carrierFreq_MHz, tx_phases, amplitude, nPoints)
configureTX = _Hahn_echo_ph.configureTX

def configureRX(SW_kHz, nPoints, nScans, nEchoes, nPhaseSteps):
    return _Hahn_echo_ph.configureRX(SW_kHz, nPoints, nScans, nEchoes, nPhaseSteps)
configureRX = _Hahn_echo_ph.configureRX

def init_ppg():
    return _Hahn_echo_ph.init_ppg()
init_ppg = _Hahn_echo_ph.init_ppg

def stop_ppg():
    return _Hahn_echo_ph.stop_ppg()
stop_ppg = _Hahn_echo_ph.stop_ppg

def ppg_element(str_label, firstarg, secondarg=0):
    return _Hahn_echo_ph.ppg_element(str_label, firstarg, secondarg)
ppg_element = _Hahn_echo_ph.ppg_element
...
def runBoard():
    return _Hahn_echo_ph.runBoard()
runBoard = _Hahn_echo_ph.runBoard

def getData(output_array, nPoints, nEchoes, nPhaseSteps, output_name):
    return _Hahn_echo_ph.getData(output_array, nPoints, nEchoes, nPhaseSteps, output_name)
getData = _Hahn_echo_ph.getData
