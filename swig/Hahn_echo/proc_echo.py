...
for date,id_string in [
    ...
    s.ift('t')
    fl.next('time domain')
    fl.plot(s, alpha=0.5, label='real')
    fl.plot(s.imag, alpha=0.5, label='imag')
    s.ft('t')
    fl.next('f')
    fl.plot(s, alpha=0.5, label='raw')
    s *= exp(1j*2*pi*10e-3*s.fromaxis('t'))
    fl.plot(s, alpha=0.5, label='shifted, real')
    fl.plot(s.imag, alpha=0.5, label='shifted, imag')
    fl.show();quit()
    s.ft('t',shift=True)
    s *= exp(1j*2*pi*0.3)
    s.ift('t')
