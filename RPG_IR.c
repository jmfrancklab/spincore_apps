...
int processArguments(int argc, char* argv[], SCANPARAMS* scanParams)
{
    ...
    scanParams->nPoints_vd = atoi (argv[3]);
    ...
    scanParams->varDelay_us = atof (argv[9]);
    ...
    scanParams->vd_step = atof (argv[14]);
    scanParams->nEchoes = atoi (argv[15]);
...
int programBoard(SCANPARAMS * scanParams)
{
    ...
    for( vd_counter = 1 ; vd_counter < scanParams->nPoints_vd ; vd_counter++){
        temp_vd = scanParams->varDelay_us*vd_counter*scanParams->vd_step;
        // SCAN LOOP
