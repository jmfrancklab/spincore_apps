...
/**
 * \file pb-Quad_Core500_example1.c
 * \brief PulseBlaster Quad-Core Example 1
 * This program is an example to generate identical 20ns pulses on all 4 cores.
 * \ingroup QuadCore
 */
...
main (void)
{
	...
	printf ("Example 1\n---------\n");
	printf ("This program is an example to generate identical three pulses of increasing\nwidth on all 4 cores.\n\n");
...
//********* Program Cores ******************-/
	pb_select_core(ALL_CORES);
	pb_start_programming (PULSE_PROGRAM);

	pb_4C_inst(1, 12.0 * ns);
	pb_4C_inst(0, 14.0 * ns);
	pb_4C_inst(1, 16.0 * ns);
	pb_4C_inst(0, 18.0 * ns);
	pb_4C_inst(1, 20.0 * ns);
	pb_4C_inst(0, 22.0 * ns);
	pb_4C_stop();
