...
/**
 * \file pb_debug.c
 * \brief This program generates a debug log.
 * \ingroup genex
 */
...
int main(int argc, char **argv)
{
	...
	pb_set_debug(1);	/*Enable the debugging log */
	...
	if (pb_init()) {
		printf
		    ("Error initializing board: %s.\nPlease check the log.txt file "
		     "that was created in the current directory.\n", pb_get_error());
		pause();     
		return -1;
	} else {
		printf
		    ("Board detected and initialized.\nPlease check the log.txt file "
		     "that was created in the current directory for more information.\n");
	}
...
int detect_boards()
{
	...
	if (numBoards <= 0) {
		printf("No Boards were detected in your system.\n\n");
		pause();
		exit(-1);
	}
