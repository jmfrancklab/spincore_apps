...
int fsweep(double maxFreq, int is160, int is3200, int allowPhase, int noPTS, double fstart, double fend, double dF, int phase, double delay);
...
int main(int argc, char* argv[])
{
    ...
    fsweep(maxFreq, is160, is3200, allowPhase, noPTS, fStart, fEnd, dF, phase, delay);
...
int fsweep(double maxFreq, int is160, int is3200, int allowPhase, int noPTS, double fstart, double fend, double dF, int phase, double delay)
{
	...
	{
		set_pts(maxFreq, is160, is3200, allowPhase, noPTS, fstart, phase);
		return 0;
	}
	...
	for(i=0; i <= count; ++i)
	{
		set_pts(maxFreq, is160, is3200, allowPhase, noPTS, fstart+i*dF, phase);
