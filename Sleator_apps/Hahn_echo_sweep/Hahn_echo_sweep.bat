...
REM *********************************************************
REM 	Hahn_echo_sweep.bat 
REM 	This file is used to run the program Hahn_echo_sweep.exe. 
REM 	Details about the specific echo parameters are described below. 

REM 	Tycho Sleator.
REM 	2013/11/26 11:35:00
REM *********************************************************
...
REM P1_TIME (microseconds): 90 degree pulse time (must be at least 0.065). 
set P1_TIME=4.2

REM P2_TIME (microseconds): 180 degree pulse time (must be at least 0.065). 
set P2_TIME=8.4

REM RINGDOWN_TIME (microseconds) (must be atleast 0.065).
set RINGDOWN_TIME=50

REM P1_PHASE (degrees): Output (Tx) phase of 90 degree pulse.  
set P1_PHASE=15.0

REM P2_PHASE (degrees): Output (Tx) phase of 180 degree pulse.
set P2_PHASE=15.0
...
Hahn_echo_sweep %FILENAME% %SPECTROMETER_FREQUENCY% %SPECTRAL_WIDTH% %P1_TIME% %P2_TIME% %RINGDOWN_TIME% %P1_PHASE% %P2_PHASE% %TAU_MIN% %DELTA_TAU% %N_TAU% %NUMBER_OF_SCANS% %REPETITION_DELAY% %AMPLITUDE% %BLANKING_DELAY% %BYPASS_FIR% %ADC_FREQUENCY% %BLANKING_BIT% %DEBUG% %BOARD_NUMBER% 
pause
