/* file Hahn_echo_sweep.c:  Last Edit 11/26/2013, 4/10/2012
 *
 *     Created by:
 *     Tycho Sleator 
 *     New York University
 ...
 * adapted from CPMG.c from spincore by Tycho Sleator. 
...
//     trigger_scan, flags, inst, inst_data, length)
  ...
  pb_inst_radio_shape (0, PHASE090, PHASE000, 1, TX_DISABLE, NO_PHASE_RESET,
    NO_TRIGGER,0,0, myScan->blankMask, CONTINUE, 0, ( myScan->ringdown_time + myScan->tau - myScan->tau_min ) * us);

  // and then start scan and wait time 2*(myScan->tau_min).
  pb_inst_radio_shape (0, PHASE090, PHASE000, 0, TX_DISABLE, NO_PHASE_RESET,
    DO_TRIGGER,0,0, myScan->blankMask, CONTINUE, 0, 2 * myScan->tau_min * us);
...
processArguments (int argc, char *argv[], SCANPARAMS * scanParams)
{
  ...
  scanParams->tau_min = atof (argv[9]);		  // us
  scanParams->delta_tau = atof (argv[10]);    // us
  scanParams->n_tau = atoi (argv[11]);
  scanParams->num_scans = atoi (argv[12]);
  scanParams->repetition_delay = atof (argv[13]); // s
  scanParams->amplitude = atof (argv[14]);  
  scanParams->blankDelay = atof (argv[15]);       // ms
  scanParams->bypass_fir = atoi (argv[16]);
  scanParams->adcFrequency = atof (argv[17]);	  // MHz
  scanParams->blankBit = atoi (argv[18]);
  scanParams->debug = atoi (argv[19]);
  scanParams->board_num = atoi (argv[20]);
