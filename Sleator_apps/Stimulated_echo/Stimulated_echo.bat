...
REM *********************************************************
REM 	Stimulated_echo.bat 
REM 	This file is used to run the program Stimulated_echo.exe. 
...
set FILENAME=stim_echo

REM SPECTROMETER_FREQUENCY (MHz) must be between 0 and 100.
set SPECTROMETER_FREQUENCY=21.2007
...
REM P3_TIME (microseconds): 90 degree pulse time must be atleast 0.065. 
set P3_TIME=4.2
...
REM P3_PHASE (degrees): Output (Tx) phase of 180 degree pulse. 
set P3_PHASE=15.0

REM TAU1 (microseconds) must be at least 0.065.  
set TAU1=20000

REM TAU2 (microseconds) must be at least 0.065.  
set TAU2=80000
...
Stimulated_echo %FILENAME% %SPECTROMETER_FREQUENCY% %SPECTRAL_WIDTH% %P1_TIME% %P2_TIME% %P3_TIME% %RINGDOWN_TIME% %P1_PHASE% %P2_PHASE% %P3_PHASE% %TAU1% %TAU2% %NUMBER_OF_SCANS% %REPETITION_DELAY% %AMPLITUDE% %BLANKING_DELAY% %BYPASS_FIR% %ADC_FREQUENCY% %BLANKING_BIT% %DEBUG% %BOARD_NUMBER% 
pause
