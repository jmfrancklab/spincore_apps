/* file file Hahn_echo.c:   Last Edit 11/26/2013 10:49
 *                                
 *     Created by:
 *     Tycho Sleator 
 *     New York University
 ...
 *
 * Adapted from file "Stimulated_echo.c", which was 
 * adapted from "CPMG.c" from spincore by the following changes:
 * Remove loop-back statement for "echo_loop", and add an additional 
 * Pulse within the loop to make a stimulated echo program.
 * 
 * See the accompanying batch file "Hahn_echo.bat" for instructions
...
//  Duration of time over which data are collected.  
    scan_time = myScan->p2_time + 3*myScan->tau; 
...
//     trigger_scan, flags, inst, inst_data, length)
 ...
 scan_loop_label =
	...
	// Start the scan and wait time tau
	pb_inst_radio_shape (0, PHASE090, PHASE000, 1, TX_DISABLE, NO_PHASE_RESET,
	   DO_TRIGGER,0,0, myScan->blankMask, CONTINUE, 0, (myScan->tau) * us);
    ...
    // total duration of scan is about 3*tau)
    pb_inst_radio_shape (0, PHASE090, PHASE000, 0, TX_DISABLE, NO_PHASE_RESET,
	   NO_TRIGGER,0,0, myScan->blankMask, CONTINUE, 0, 2 * myScan->tau * us);
...
processArguments (int argc, char *argv[], SCANPARAMS * scanParams)
{
  ...
  scanParams->tau = atof (argv[9]);		      // us
  scanParams->num_scans = atoi (argv[10]);
  scanParams->repetition_delay = atof (argv[11]); // s
  scanParams->amplitude = atof (argv[12]);  
  scanParams->blankDelay = atof (argv[13]);       // ms
  scanParams->bypass_fir = atoi (argv[14]);
  scanParams->adcFrequency = atof (argv[15]);	  // MHz
  scanParams->blankBit = atoi (argv[16]);
  scanParams->debug = atoi (argv[17]);
  scanParams->board_num = atoi (argv[18]);
